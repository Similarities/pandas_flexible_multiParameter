...
my_sorted_data.switch_param_and_condition('EL on target', 2.7)
my_sorted_data.low_pass()
print("xxxxxxxxx energy range on target 1.7-2.7")
my_sorted_data.switch_param_and_condition("z um", -1400)
my_sorted_data.low_pass()

PP_out_energy_range = my_sorted_data.return_df()
print("only !!! shots considered with z<-1400 um")
...
number_too_low_cutoff = len(xprime[["div25 gauss"]]) - len(x[['div25 gauss']])
x[['div25 gauss']] = x[['div25 gauss']].replace(0., np.nan)

print("not evaluable or Nmax<24 in selection:", x[['div25 gauss']].isna().sum())
print("not used shots general ratio:", (x[['div25 gauss']].isna().sum() + number_too_low_cutoff) / len(xprime[['div25 gauss']]))
...
bandpass_all2 = bandpass_selection(PP_out_energy_range, 'GVD in fs^2', 400, 650)
bandpass_all2.band_pass_1()
xprime = bandpass_all2.keep_this_selection(True)
print( "number of shots in this range including lower cutoff", len(xprime[["div25 gauss"]]))
print("all shots in this selection, Nmax>24:", len(xx[['div25 gauss']]))
number_too_low_cutoff = len(xprime[["div25 gauss"]]) - len(xx[['div25 gauss']])
print("number of too low cutoff shots:", number_too_low_cutoff)
xx[['div25 gauss']] = xx[['div25 gauss']].replace(0., np.nan)
print("not evaluable or Nmax<24 in selection:", xx[['div25 gauss']].isna().sum())
print("not used shots general ratio:", (xx[['div25 gauss']].isna().sum() + number_too_low_cutoff) / len(xprime[['div25 gauss']]))
...
bandpass3 = bandpass_selection(PP_out, 'GVD in fs^2', 700, 1105)
xxx = bandpass3.band_pass_1()
bandpass3.keep_this_selection(True)
bandpass_all3 = bandpass_selection(PP_out_energy_range, 'GVD in fs^2', 700, 1105)
bandpass_all3.band_pass_1()
xprime = bandpass_all3.keep_this_selection(True)
print( "number of shots in this range including lower cutoff", len(xprime[["div25 gauss"]]))
print("all shots in this selection, Nmax>24:", len(xxx[['div25 gauss']]))
number_too_low_cutoff = len(xprime[["div25 gauss"]]) - len(xxx[['div25 gauss']])
print("number of too low cutoff shots:", number_too_low_cutoff)
...
print("not used shots general ratio:", (xxx[['div25 gauss']].isna().sum() + number_too_low_cutoff) / len(xprime[['div25 gauss']]))
