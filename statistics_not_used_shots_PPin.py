...
my_sorted_data.switch_param_and_condition('EL on target', 1.8)
my_sorted_data.high_pass()
my_sorted_data.switch_param_and_condition('EL on target', 2.4)
my_sorted_data.low_pass()
my_sorted_data.switch_param_and_condition('z um', -600)
my_sorted_data.high_pass()
my_sorted_data.switch_param_and_condition('z um', 600)
my_sorted_data.low_pass()
PP_in_all = my_sorted_data.return_df()

my_sorted_data.switch_param_and_condition("Nmax", 24)
my_sorted_data.high_pass()
#my_sorted_data.low_pass()

PP_N24 = my_sorted_data.return_df()

#print(PP_out[ ["divergence N=25", "PP in out", 'Nmax', 'z um', 'day', 'shot']])
#print(len(PP_out), " selected of:", len(RESULT))

print("energy range:", 1.8, "to", 2.3)
print("only shots with target within Ry (pm 600um) considered")

bandpass = bandpass_selection(PP_in_all, 'GVD in fs^2', -300, 400)
bandpass.band_pass_1()
x = bandpass.keep_this_selection(True)
print("all shots in this selection:", len(x[['Nmax']]))
bandpass_over = bandpass_selection(PP_N24, 'GVD in fs^2', -300, 400)
bandpass_over.band_pass_1()
x = bandpass_over.keep_this_selection(True)
print("shots Nmax>24 in Ry:", len(x[['Nmax']]))
...
bandpass2 = bandpass_selection(PP_in_all, 'GVD in fs^2', 400, 650)
xx = bandpass2.band_pass_1()
print("all shots in this selection:", len(xx[['Nmax']]))
bandpass_2 = bandpass_selection(PP_N24, 'GVD in fs^2', 400, 650)
bandpass_2.band_pass_1()
xx = bandpass_2.keep_this_selection(True)
print("shots Nmax>24 in Ry:", len(xx[['Nmax']]))
...
bandpass3 = bandpass_selection(PP_in_all, 'GVD in fs^2', 700, 1200)
xxx = bandpass3.band_pass_1()
bandpass3.keep_this_selection(True)
print("all shots in this selection:", len( xxx[['Nmax']]))
bandpass_3 = bandpass_selection(PP_N24, 'GVD in fs^2', 700, 1200)
xxx = bandpass_3.band_pass_1()
bandpass_3.keep_this_selection(True)
print("shots Nmax>24 in Ry:", len( xxx[['Nmax']]))
