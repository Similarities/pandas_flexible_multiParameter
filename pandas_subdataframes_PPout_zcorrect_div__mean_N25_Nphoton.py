...
experiments = pd.read_excel('fundamentalshifts12_selection_div_new.xlsx')
...
my_sorted_data.switch_param_and_condition("EL on target", 2.3)
...
print(x[['divergence N=25', 'shot', 'z um']])

mean_x, mean_y, erry = mean_of_something_vs_something(x, stepsize_x, 1, "z um", "Nphoton_sr_bw", "[Nphoton/sr 0.1%bw]",
                                                      "GDD -300 - 400 fs^2")
plot_together_in_one_graph("c", "GDD -200- 300 fs^2", mean_x, mean_y, "z [um]", "[Nphoton/sr 0.1%bw]", errx, erry, "*")
print(x[['shot', 'day', 'Nphoton_sr_bw', 'z um', 'GVD in fs^2']])
print('number of entries for  GDD~0', len(mean_y))

bandpass2 = bandpass_selection(PP_out, 'GVD in fs^2', 350, 890)
xx = bandpass2.band_pass_1()
xx[['divergence N=25']] = xx[['divergence N=25']].replace(0., np.nan)
mean_x, mean_y, erry = mean_of_something_vs_something(xx, stepsize_x, 1, "z um", "Nphoton_sr_bw", "[Nphoton/sr 0.1%bw]",
                                                      "GDD 400 - 700 fs^2")
plot_together_in_one_graph("r", "GDD 400- 700 fs^2", mean_x, mean_y, "z [um]", "[Nphoton/sr 0.1%bw]", errx, erry, "o")
print(xx[['shot', 'day', 'Nphoton_sr_bw', 'z um', 'GVD in fs^2']])
print('number of entries for  GDD~600', len(mean_y))

bandpass3 = bandpass_selection(PP_out, 'GVD in fs^2', 890, 1300)
xxx = bandpass3.band_pass_1()
xxx[['divergence N=25']] = xxx[['divergence N=25']].replace(0., np.nan)

mean_x, mean_y, erry = mean_of_something_vs_something(xxx, stepsize_x, 1, "z um", "Nphoton_sr_bw",
                                                      "[Nphoton/sr 0.1%bw]", "GDD 900 - 1200 fs^2")
plot_together_in_one_graph("black", "GDD 900 - 1200 fs^2", mean_x, mean_y, "z [um]", "[Nphoton/sr 0.1%bw]", errx, erry, "P")
print(xxx[['shot', 'day', 'Nphoton_sr_bw', 'z um', 'GVD in fs^2']])
...
plt.xlabel('z [um]')
plt.title('1.7 - 2.8 J EL')
plt.ylabel('Nphoton/sr @ 0.1%bw N25 +/- 0.3')
plt.legend()
save_picture('N25_Nphoton_paper1.7-2.3J')
