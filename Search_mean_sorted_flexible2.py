...
class Sorting_by_parameters:
    ...
    def __init__(self,  database, name, save_bool):
        ...
        self.GVD300 = self.copy['GVD in fs^2'] > 300 #ok.
        self.GVD600 = self.copy['GVD in fs^2'] > 300
        self.GVD600_2 = self.copy['GVD in fs^2'] < 700
        self.GVD900 = self.copy['GVD in fs^2'] >= 700
        ...
        self.z_500 = self.copy['z um'] <= 500.
        self.z_neg = self.copy['z um'] < -400.
        self.z_1500 = self.copy['z um'] <= 1500.
        self.z_2500 = self.copy['z um'] <= 2500.
        self.z_3500 = self.copy['z um'] > 2500.
        self.z_4000 = self.copy['z um'] > 3500.
        self.auswahl_GVD0 = []
        self.auswahl_GVD600 = []
        self.auswahl_GVD900 = []
        self.auswahl_GVDneg = []
    ...
    def reset_auswahl(self):
        ...
        return self.auswahl, self.auswahl1, self.auswahl2, self.auswahl3, self.auswahl4
    ...
    def z_selection_dataframe(self):

        self.reset_auswahl()


        self.auswahl1 = self.copy[self.z_neg][self.copy.columns]


        self.auswahl2= self.copy[self.z_500 != self.z_neg][self.copy.columns]



        self.auswahl3 = self.copy[self.z_1500 != self.z_500][self.copy.columns]



        self.auswahl = self.copy[self.z_2500 != self.z_1500][self.copy.columns]

        self.auswahl4 = self.copy[self.z_3500 ][self.copy.columns]


        print(self.auswahl1[["shot", "z um", "GVD in fs^2"]], "that was z<-400")

        print(self.auswahl2[["shot", "z um", "GVD in fs^2"]], "that was z> -400 z<=+ 500 um")

        print(self.auswahl3[["shot", "z um", "GVD in fs^2"]], "that was z >500 & <=1500")

        print( self.auswahl[["shot", "z um", "GVD in fs^2"]], "that was z >1500 & <=2500")\

        print( self.auswahl4[["shot", "z um", "GVD in fs^2"]], "that was z >2500 ")

        # test would be to test in sorted list for logic numbers ><


        self.selection_members = ("z-1000 - -400", "z -400 - +500","z 600 - 1500","z 1600-2500", 'z >2500 ' )


        return self.auswahl1, self.auswahl2, self.auswahl3, self.auswahl, self.auswahl4, self.selection_members
    ...
    def count_auswahl_by_parameter_for_parameter2(self,  coloumlabel_param1, number, coloumblabe_param2):

        #self.GVDneg = self.auswahl4['GVD in fs^2'] < 0

        selection = self.auswahl4.copy()[[coloumlabel_param1, coloumblabe_param2, 'shot', 'day', 'z um']]



        selection = selection[selection['GVD in fs^2'] < 301][[coloumlabel_param1, coloumblabe_param2]]
        selection = selection[selection['GVD in fs^2'] >= -200][[coloumlabel_param1, coloumblabe_param2]]

        number_All = selection.count()[coloumblabe_param2]

        selection = selection[selection['Nmax'] >= 22][[coloumlabel_param1, coloumblabe_param2]]
       # print(CWE_selection, 'selection')



        number_CWE = selection[selection['Nmax'] >= 22].count()[coloumblabe_param2]
        number_ROM = selection[selection['Nmax'] >= 25].count()[coloumblabe_param2]

        print(number_CWE,'xxxxxxxxxxx', number_All, "xxxxxxxxxxxxxxxxxx")

        ratio = number_CWE/number_All
        ratio_ROM = number_ROM/number_All

        print(ratio, "% for N_max>21")
        print(ratio_ROM, "% for N_max >=25")

        print(self.selection_members)


        #print(self.auswahl4[[coloumlabel_param1, coloumblabe_param2]]) for testing


        # log and test criteria!





        #print(self.auswahl4[self.GVD0 !=self.GVD0_2 & self.N_CWE ].count()[coloumlabel_param1], ".count for range on parameter 1")


        #single parameter logical

        #print(self.auswahl4[self.auswahl4[coloumlabel_param1] == number].sum(), 'that was .sum() doing it on all coloums')
        #print(self.auswahl4[self.auswahl4[coloumlabel_param1] == number].count()[coloumblabe_param2], 'that was .count() doing it on only param2')


        mean_param2 = self.auswahl4[self.auswahl4[coloumlabel_param1] == number].mean()[coloumblabe_param2]

        std_param2 = self.auswahl4[self.auswahl4[coloumlabel_param1] == number].std(axis = 0)[coloumblabe_param2]
        print(mean_param2, "mean value on parameter 2", coloumblabe_param2)
        print(std_param2, "standard deviation on parameter 2", coloumblabe_param2)

        #if N_max groesser als N22 ... dann mean value ...
...
sorting1 = Sorting_by_parameters(RESULT, "SHHG19_", False)
...
### double_sorting ##
sorting1.z_selection_dataframe()
sorting1.count_auswahl_by_parameter_for_parameter2("GVD in fs^2", 600, "Nmax")
