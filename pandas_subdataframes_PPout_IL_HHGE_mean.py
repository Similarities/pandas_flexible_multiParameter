...
def theoretical_cal(liste_z, liste_E, liste_GVD, tauL): 
    
    liste_I =[0]*len(liste_z)
    #wrong format: writes as complex numbers into df (?)
 
    
    from theoretical_I import I_calc
    
    
    for i in range(0, len(liste_z)):
        
        liste_GVD[i] = liste_GVD[i]*1E-30
        
        tauL_i = (((tauL**4)+ 16*(0.69**2) *(liste_GVD[i]**2))**0.5)/tauL
        
        aha = I_calc(liste_z[i], tauL_i, liste_E[i])

        liste_I[i] = aha.calc_A_and_I()*1E-19
        liste_I[i] = round(liste_I[i],3)

        
        
        #print(liste_I[i], tauL_i, 'I', 'tauC')
...
new = mam.export_one_column('z um')
EL = mam.export_one_column('EL on target')
z =  mam.export_one_column('z um')
GVD = mam.export_one_column('GVD in fs^2')
IL = theoretical_cal(z, EL, GVD, 20*1E-15)
print(IL, ' intensity per shot')

mam.new_column(IL, "IL")
...
stepsize_x = stepsize_x_axis( 0, 30000)
...
#print(x['IL'])
mean_x, mean_y, erry = mean_of_something_vs_something(x, stepsize_x, 0.0001, "IL", "HHG 24nm-34nm", "[uJ]","GDD -300 - 300 fs^2"  )
plot_together_in_one_graph("c", "GDD -200- 300 fs^2", mean_x, mean_y, "IL 1E19 [W/cm^2]", 'HHG 24-34nm [uJ]', errx, erry)


bandpass2 = bandpass_selection(PP_out, 'GVD in fs^2', 400, 900)
xx = bandpass2.band_pass_1()
mean_x, mean_y,  erry = mean_of_something_vs_something(xx, stepsize_x, 0.0001, "IL", "HHG 24nm-34nm", "[uJ]","GDD 400 - 700 fs^2"  )
plot_together_in_one_graph("r", "GDD 400- 700 fs^2", mean_x, mean_y, "IL 1E19[W/cm^2]", 'HHG 24-34nm [uJ]', errx, erry)
...
mean_x, mean_y, erry = mean_of_something_vs_something(xxx, stepsize_x, 0.0001, "IL", "HHG 24nm-34nm", "[uJ]","GDD 900 - 1200 fs^2"  )
plot_together_in_one_graph("black", "GDD 900 - 1200 fs^2", mean_x, mean_y, "IL 1E19[W/cm^2]", 'HHG 24-34nm [uJ]', errx, erry)
...
plt.xlim(5E-4, 3)
plt.ylim(-0.05, 0.3)
save_picture('Mean_HHGE_vs_IL_high')
plt.show()
